"""Core data models for AI News XHS Publisher."""
...
class OpenBBConfig(BaseModel):
    ...
    settings file, so this app does not need to pass them explicitly.
    """
...
class EmailConfig(BaseModel):
    ...
    sender_name: str = "AI News XHS Daily"
    subscribe_keyword: str = "SUBSCRIBE"
    unsubscribe_keyword: str = "UNSUBSCRIBE"
    enabled: bool = False


class XiaohongshuImageConfig(BaseModel):
    """Image generation settings for Xiaohongshu notes."""
    ...
    output_dir: str = "data/xiaohongshu/images"
    count: int = 1
    prompt_template: str = (
        "Create a clean editorial Xiaohongshu cover image for a technology news "
        "briefing. Use a modern magazine-style composition, no logos, no tiny text. "
        "Title: {title}\nKey points:\n{items}"
    )


class XiaohongshuConfig(BaseModel):
    """Xiaohongshu publishing configuration.

    The app can either send a structured note to a user-provided endpoint or
    call the bundled local CDP publishing pipeline.
    """

    enabled: bool = False
    publish_mode: Literal["endpoint", "local_cdp"] = "endpoint"
    endpoint_env: Optional[str] = None
    ...
    title_template: str = "AI 新闻每日速递 {date}"
    tags: List[str] = Field(default_factory=lambda: ["AI新闻", "科技资讯", "每日资讯"])
    post_source: Literal["summary", "top_scored_item"] = "summary"
    polish_with_ai: bool = False
    title_max_chars: int = 28
    polish_prompt: str = (
        "你是一个懂科技趋势和小红书传播的中文内容编辑。请基于素材写一篇小红书图文笔记："
        "标题要有点击率但不能标题党；正文要更有深度、更有趣、能引发讨论；"
        "必须解释这条新闻为什么重要、可能影响谁、普通读者可以怎么理解；"
        "结尾给一个开放式问题引导评论。"
    )
    image_generation: XiaohongshuImageConfig = Field(default_factory=XiaohongshuImageConfig)
    image_paths: List[str] = Field(default_factory=list)
    image_urls: List[str] = Field(default_factory=list)
    video_path: Optional[str] = None
    video_url: Optional[str] = None
    local_pipeline_path: Optional[str] = None
    local_pipeline_env: str = "XIAOHONGSHU_PIPELINE"
...
class Config(BaseModel):
    ...
    xiaohongshu: Optional[XiaohongshuConfig] = None
