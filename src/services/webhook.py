"""Webhook notification service for AI News XHS."""
            ...
            "card": {
                ...
                "header": {
                    "title": {
                        "tag": "plain_text",
                        "content": (
                            f"AI News XHS {date} 折叠日报"
                            if lang == "zh"
                            else f"AI News XHS {date} Collapsible Daily"
                        ),
                    },
                    "template": "blue",
                },
    ...
    def build_daily_summary_messages(
        ...
        if delivery == "summary_and_items":
            ...
            overview_message = {
                **base_vars,
                "message_title": (
                    f"AI News XHS {date} 总览"
                    if lang == "zh"
                    else f"AI News XHS {date} Overview"
                ),
        ...
        return [
            {
                **base_vars,
                "message_title": (
                    f"AI News XHS {date} 日报" if lang == "zh" else f"AI News XHS {date} Daily"
                ),
        ...
        await self.notify(
            {
                ...
                "result": "failed",
                "timestamp": str(int(datetime.now(timezone.utc).timestamp())),
                "message_title": "AI News XHS generation failed",
